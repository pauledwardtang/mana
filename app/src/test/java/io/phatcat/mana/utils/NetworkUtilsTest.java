...
public class NetworkUtilsTest {
    ...
    @Test
    public void testConvertFromNetworkToModel() {
        Type listType = new TypeToken<List<Recipe>>(){}.getType();
        List<Recipe> recipeList = new Gson().fromJson(SAMPLE_STRING, listType);

        List<RecipeData> recipeData = NetworkUtils.from(recipeList);

        assertNotNull(recipeData);
        assertThat(recipeData.size(), is(4));
        
        RecipeData firstRecipe = recipeData.get(0);
        assertThat(firstRecipe.recipe.id, is(1L));
        assertThat(firstRecipe.recipe.name, is("Nutella Pie"));
        assertThat(firstRecipe.recipe.servings, is(8.0));
        assertThat(firstRecipe.recipe.imageUrl, is(""));

        // Test some ingredients and steps
        assertThat(firstRecipe.recipeIngredients.size(), is(9));

        RecipeIngredientsData thirdIngredientData = firstRecipe.recipeIngredients.get(2);
        assertThat(thirdIngredientData.getQuantity(), is(0.5));
        assertThat(thirdIngredientData.getMeasure().shortName, is("CUP"));
        assertThat(thirdIngredientData.getIngredient().name, is("granulated sugar"));

        assertThat(firstRecipe.recipeSteps.size(), is(7));

        Step lastStep = firstRecipe.recipeSteps.get(firstRecipe.recipeSteps.size() - 1);
        assertThat(lastStep.stepNo, is(6));
        assertThat(lastStep.shortDescription, is("Finishing Steps"));
        assertThat(lastStep.description, is("6. Pour the filling into the prepared crust and smooth the top. Spread the whipped cream over the filling. Refrigerate the pie for at least 2 hours. Then it's ready to serve!"));
        assertThat(lastStep.videoUrl, is("https://d17h27t6h515a5.cloudfront.net/topher/2017/April/58ffda45_9-add-mixed-nutella-to-crust-creampie/9-add-mixed-nutella-to-crust-creampie.mp4"));
        assertThat(lastStep.thumbnailUrl, is(""));
    }
